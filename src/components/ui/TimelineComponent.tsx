...
export default function TimelineComponent({
  ...
  return (
    <section ref={ref} className="flex flex-col relative">
      {/* Header Section */}
      ...
      {/* Timeline Items */}
      <div className="flex flex-col z-20 pt-5 shadow-md hover:shadow-[#354274] lg:hover:shadow-none relative border border-t-0 border-[#354274] rounded-b-md bg-[radial-gradient(ellipse_at_center,#1a213b,#1a213b,transparent)] lg:border-none lg:bg-none">
        {items.map((item, i) => (
          <div
            key={i}
            className="timeline-item flex gap-[26px] items-start relative pb-6 lg:pb-[52px]"
            style={{ animationDelay: `${i * 0.2}s` }}
          >
            {" "}
            {/* Vertical Line */}
            {i !== items.length - 1 && (
              <div className="absolute left-[21px] top-4 bottom-0 w-px bg-white lg:bg-brand-500 z-10 timeline-line" />
            )}
            {/* Pointer */}
